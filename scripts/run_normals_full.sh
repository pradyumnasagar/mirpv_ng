...
shopt -s nullglob
FASTQS=( "$FASTQ_DIR"/normal*.fastq "$FASTQ_DIR"/normal*.fastq.gz "$FASTQ_DIR"/normal*.fq "$FASTQ_DIR"/normal*.fq.gz )
shopt -u nullglob
(( ${#FASTQS[@]} > 0 )) || { echo "[ERROR] No normal*.fastq found in $FASTQ_DIR" >&2; exit 1; }

echo "[INFO] Found ${#FASTQS[@]} normal FASTQ(s) in $FASTQ_DIR"
...
echo
echo "[ALL DONE] Normals complete: $OUT_ROOT"
