...
"""
Feature extraction module for miRPV-NG.

This module provides functions to compute structural and sequence features
from RNA sequences and their secondary structures. Features are used for
machine learning classification of pre-miRNA candidates.

Feature Sets:
    - **core36**: Compact baseline feature set (~15 features)
    - **extended**: Full feature set including dinucleotides, PGS, and tier2 (~80+ features)

Example:
    >>> from mirpv_ng.features import run_rnafold, extended_features
    >>> struct, mfe = run_rnafold("GGCCAUUAGGCC")
    >>> feats = extended_features("GGCCAUUAGGCC", struct, mfe)
"""
...
def run_rnafold(seq: str, rnafold_bin: str = "RNAfold") -> Tuple[str, float]:
    ...
    Computes the minimum free energy (MFE) secondary structure prediction
    using the ViennaRNA RNAfold program.
    
    Args:
        seq: RNA/DNA sequence (T is automatically converted to U).
        rnafold_bin: Path to RNAfold executable. Defaults to "RNAfold".
    
    Returns:
        Tuple of (structure, mfe) where:
            - structure: Dot-bracket notation string (e.g., "(((...)))")
            - mfe: Minimum free energy in kcal/mol (negative values are stable)
    
    Raises:
        RuntimeError: If RNAfold fails or produces unexpected output.
    ...
    non_acgu = re.sub(r'[ACGU]', '', seq)
    if non_acgu:
        logger.warning(
            "[RNAfold] Input contains %d non-ACGU character(s): %s",
            len(non_acgu), set(non_acgu),
        )
...
def core36_features(seq: str, struct: str, mfe: float) -> Dict[str, float]:
    """
    Compute the core baseline feature set for RNA hairpin classification.
    
    This compact feature set includes ~15 features covering sequence composition,
    secondary structure statistics, and thermodynamic properties.
    
    Args:
        seq: RNA sequence (T is converted to U internally).
        struct: Dot-bracket secondary structure notation.
        mfe: Minimum free energy from RNAfold (kcal/mol).
    
    Returns:
        Dictionary mapping feature names to float values. Keys include:
            - len: Sequence length
            - gc_frac, au_frac: GC and AU content fractions
            - num_pairs, paired_frac: Base pair statistics
            - num_loops, mean_loop_size, max_loop_size: Loop statistics
            - num_bulges, bulge_density: Bulge statistics
            - mfe, mfe_per_nt: Thermodynamic features
    
    Example:
        >>> feats = core36_features("GGCCAUUAGGCC", "((((....))))", -5.4)
        >>> print(feats["gc_frac"])  # ~0.67
    """
...
def extended_features(seq: str, struct: str, mfe: float, tier2_cfg: Tier2Config = Tier2Config(enabled=False)) -> Dict[str, float]:
    """
    Compute the extended feature set for RNA hairpin classification.
    
    This comprehensive feature set (~80+ features) includes all core36 features
    plus dinucleotide frequencies, trinucleotide motifs, segment-wise GC/pairing,
    pair composition, helix statistics, bulge features, PGS features, and
    optional tier-2 soft-gating features.
    
    Args:
        seq: RNA sequence (T is converted to U internally).
        struct: Dot-bracket secondary structure notation.
        mfe: Minimum free energy from RNAfold (kcal/mol).
        tier2_cfg: Configuration for tier-2 soft-gating features.
            Defaults to disabled.
    
    Returns:
        Dictionary mapping feature names to float values. Includes:
            - All core36 features
            - di_*: 16 dinucleotide frequencies (di_AA, di_AC, ...)
            - tri_*: Selected trinucleotide motifs (tri_UGU, tri_GCG, ...)
            - seg*_gc, seg*_paired_frac: 4-segment features
            - pgs_*: Position-specific geometric scoring features
            - tier2_*: Soft-gating penalty features (if enabled)
    
    Example:
        >>> feats = extended_features(seq, struct, mfe)
        >>> print(len(feats))  # ~80+ features
    """
    feats: Dict[str, float] = {}
        ...
        for line in f:
            ...
            if line.startswith(">"):
                if current_id is not None:
                    ...
                    bad = set(joined.upper()) - _VALID_BASES
                    if bad:
                        logger.warning(
                            "[read_fasta] Seq '%s' contains non-ACGTU chars: %s",
                            current_id, bad,
                        )
                    records.append((current_id, joined))
