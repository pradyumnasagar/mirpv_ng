...
def write_rejects_header(path: Path) -> None:
    # Always overwrite on each run (prevents stale reasons from older runs)
...
def _norm_repeat_class(x: str) -> str:
    if x is None:
        return ""
    x = str(x).strip()
    if not x or x.upper() in {"NA", "NONE", "NO", "NAN"}:
        return ""
    # RepeatMasker sometimes gives things like "LTR/ERV1" or "SINE/Alu"
    x = x.split("/")[0].strip().upper()
    return x
...
def decision_ladder(
    ...
    final_label:
      ...
      - Reject
    """
    rc = _norm_repeat_class(repeat_class)

    block = set(_norm_repeat_class(x) for x in (args.repeat_block or []))
    allow = set(_norm_repeat_class(x) for x in (args.repeat_allow or []))

    # 1) Allow overrides everything
    if allow and rc in allow:
        pass  # do not block even if it appears in block

    # 2) If allowlist is provided and rc not in it -> reject
    elif allow and rc not in allow:
        return ("Reject", f"RepeatNotAllowed:{rc}")

    # 3) Otherwise apply blocklist
    elif rc in block:
        return ("Reject", f"RepeatBlocked:{rc}")

    # Choose RF thresholds depending on repeat status
    if rc and rc != "None":
...
def run_peaks_to_finalists(args: argparse.Namespace) -> int:
    ...
    write_rejects_header(out_rejects)
    ...
    with peaks_scored.open("r", encoding="utf-8", newline="") as f:
        ...
        with out_candidates.open("w", encoding="utf-8", newline="") as fo, out_strict.open("w", encoding="utf-8", newline="") as fs:
            ...
            for row in r:
                ...
                repeat_class = _norm_repeat_class(row.get("repeat_class", "")) or "None"
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    # Repeat handling
    ap.add_argument("--repeat-block", nargs="*", default=["Satellite", "Simple_repeat", "Low_complexity"])
