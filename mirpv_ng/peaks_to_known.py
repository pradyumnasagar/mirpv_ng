...
def run_peaks_to_known(args: argparse.Namespace) -> int:
    ...
    with peaks_tsv.open("r", encoding="utf-8", newline="") as f:
        ...
        if "peak_id" not in fieldnames:
            fieldnames.append("peak_id")

        required = {"sample_id", "chrom", "strand", "peak_center0"}
        ...
        with out_peaks.open("w", encoding="utf-8", newline="") as fo, out_hits.open("w", encoding="utf-8", newline="") as fh:
            ...
            for row in r:
                n_total += 1
                # Synthesize peak_id if not present in peaks.tsv (v2 fastq-to-peaks doesn't emit it)
                if not row.get("peak_id"):
                    row["peak_id"] = (
                        f"{row.get('sample_id','NA')}|"
                        f"{row.get('island_id','NA')}|"
                        f"{row.get('peak_center0','NA')}|"
                        f"{row.get('anchor_type','NA')}"
                    )
